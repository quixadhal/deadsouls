...
mixed eventMount(object who){
    ...
    if(rider_weight + this_object()->GetCarriedMass() > this_object()->GetMaxCarry()){
        return write("This chamber cannot handle that much "+weight+".");
    }
    else {
        string int_desc = GetChamberInterior();
...
mixed GetChamberInterior(){
    mixed ret;
    if(stringp(ChamberInterior)) return ChamberInterior;
    else if(functionp(ChamberInterior)){
        ret = evaluate(ChamberInterior);
