...
**    o unless specified otherwize, forces -l flag on LIST and -1 flag on
NLST.
**    o directories report size 0.
**  o If home directory does not exist, it defaults to "/".
**
** Zaxan@HavenMUD (shippy@nmt.edu) Dec 18, 2002
**  o Made small changes to make it work with Haven's lib
**  o Added more logging information and fixed some broken ones
**  o Added PASV support
**  o Fixed anonymous access
**  o Changed what directories a person is allowed to access
**
** Zaxan@HavenMUD (shippy@nmt.edu) July 4, 2004
**  o Fixed up PASV to get rid of upload delays
**  o Added ability to view the root dir
**  o Permissions are now based on the imm's in-game permissions
**
*/
...
nomask static int check_privs(string file, string oper) {
    ...
    if(file == "/ftp" || sscanf(file, "/ftp/%*s")) return 1;
    ...
    catch(ob = master()->player_object(Session->user));

    if(!undefinedp(ob))
    {
	string func = "valid_"+oper;

	if(catch(ret = call_other(master(), func, file, ob, 0)))
	{
	    eventWrite("550 Failure checking permissions.  Please report what you
were doing "
                        "to the mud admin.\n",0);
	    return 0;
	}
...
break;
   ...
   PassivePort = port;
   FTP_PORT_TRACKER_D->AddUsedPort(PassivePort);
...
void Destruct(){
   FTP_PORT_TRACKER_D->RemoveUsedPort(PassivePort);
...
tc("arg: "+arg,"yellow");
    ...
    if(member_array(arg, ({"anonymous", "ftp"})) != -1){
	  ...
	  eventWrite("530 Anonymous access denied.\n", 0);
       return;
    }
...
private void eventCmdPswd(string arg){
    ...
    if(member_array(lower_case(Session->user), ({"anonymous", "ftp"})) != -1
&& ALLOW_ANON_FTP){
        eventWrite("230 guest login ok, access restrictions apply.\n",0);
        Session->connected = 1;
        Session->priv = 0;
        Session->pwd = ANON_PREFIX;
        log_file("inet/ftp_connect", sprintf("Anomymous login "
                 "(email = %s) at %s\n", arg, ctime(time()))); 
      return;
    }
    ...
    if (Retired)
    {
	log_file("inet/ftp_connect", sprintf("Attempted logon as "
		"%s (RETIRED) at %s\n", Session->user, ctime(time())));
	eventWrite("530 Login incorrect.\n", 1);
	return;
    }
    if(!Password || Password != crypt(arg, Password) ) {
        log_file("inet/ftp_connect", sprintf("Attempted login as "
		 "%s at %s\n", Session->user, ctime(time())));
    ...
    {
       log_file("inet/ftp_connect", sprintf("Login as %s at %s\n",
	     Session->user, ctime(time())));
    }
...
return;
    ...
    Session->dataPipe->SetClose((:eventWrite("226 Transfer
complete.\n",0):));
...
:));
   eventWrite(sprintf("227 Entering Passive Mode (%s,%d,%d)\n",
	    implode(explode(HOST_IP, "."), ","), PassivePort >> 8, PassivePort &
0x00FF));
   return;
}
...
if(file_size(arg) == -2) {      
  ...
  /* in case of NLST imply -1 */
    ...
    eventWrite("150 Opening ASCII mode data connection for file
list.\r\n",0);
...
private void eventCmdRetr(string arg){
    ...
    if(!arg){ 
        eventDestructDataPipe((:eventWrite("500 command not
understood.\n",0):)); 
        return; 
    }
    ...
    switch(i){
        case -2:
            eventWrite(sprintf("550 %s: Can't retrieve (it's a
directory).\n",
                               target_file),0);
            eventDestructDataPipe(0);
            return;
    ...
    log_file("inet/ftp_transfer",
	  sprintf("%s downloaded %s at %s.\n", Session->user, target_file,
ctime(time())));
    switch(Session->binary){
        case 0:	
            outfile[Session->dataPipe]=({target_file,0,0,Session->cmdPipe,
i});
...
Session->dataPipe->eventWrite(RetrieveCmdCallback(Session->dataPipe));
            break;
        case 1:
            outfile[Session->dataPipe]=({target_file,1,0,Session->cmdPipe,
i});
...
if( file_exists(arg) ){
    ...
    Session->filepos = 0;
    log_file("inet/ftp_transfer",
	  sprintf("%s uploaded %s at %s.\n", Session->user, absolute_path(arg),
ctime(time())));
...
void eventRead(string data){
    ...
    if (!dispatchTo){
        log_file("inet/ftp_error",
                 sprintf("ftp - unknown command: %s at
%s\n",cmd,ctime(time()))); 
