...
void create() {
    ...
    set_short( "Cleric's Hospital of Praxis");
    set_long(
...
int clean_poison(string str) {
    ...
    tp->add_money("gold", -currency_value(50, "gold"));
    tp->add_poisoning(-10);
...
int regenerate(string limb) {
    ...
    */
    ...
    if(member_array(limb, (string *)RACE_D->query_wielding_limbs((string)tp->query_race())) != -1) 
...
int donate(string str) {
    ...
    this_player()->add_money("gold", currency_value(amount/5, "gold"));
    blood[what] += amount;
...
int transfuse(string str) {
    ...
    this_player()->add_money("gold", -currency_value(amount*3, "gold"));
    call_other(this_player(), "add_"+what, amount);
