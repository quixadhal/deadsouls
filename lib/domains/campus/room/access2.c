...
static void create() {
    ...
    SetRead("sign","Campus domain special room access port: FURNACE");
    SetEnters( ([
                ({"hole","port","porthole","access port"}):"/domains/campus/room/furnace"
                ]) );
    AddExit("north","/domains/campus/room/access1");
    AddExit("south","/domains/campus/room/access3");
    SetObviousExits("n,s");
}
