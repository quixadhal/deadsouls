        ...
        switch (ip->state) {
            ...
            case TS_SB:
                ...
                if (ip->sb_pos < ip->sb_size - 1)
                	...
                	ip->sb_buf = REALLOC(ip->sb_buf, ip->sb_size);
                }
                break;
            ...
            case TS_SB_IAC:
				...
				env_buf[j] = 0;
                            ...
                            case TELOPT_ZMP:
                            {
                            	...
                            	ip->sb_buf = REALLOC(ip->sb_buf, MAX(ip->sb_pos + 2, SB_SIZE));
                            	ip->sb_buf[ip->sb_pos] = 0;
...
static void new_user_handler (int which)
{
    ...
    master_ob->interactive->sb_buf = MALLOC(SB_SIZE);
...
void f_send_zmp(){
	...
	add_binary_message(current_object, (sp-1)->u.string, strlen((sp-1)->u.string));
	int i;
	char zero = 0;
	for(i=0; i<sp->u.arr->size; i++){
		if(sp->u.arr->item[i].type == T_STRING){
			add_binary_message(current_object, &zero, 1);
			add_binary_message(current_object, sp->u.arr->item[i].u.string, strlen(sp->u.arr->item[i].u.string));
		}
	}
	add_binary_message(current_object, &zero, 1);
