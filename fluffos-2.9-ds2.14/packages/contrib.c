...
/* 
 * Something in terminal_colour() broke between 2.7 and 2.9,
 * and this is a throwback to the way it used to work.
 */
#define MAP_POINTER_HASH(x) (((POINTER_INT)x) >> 4)

void
f_terminal_color (void)
{
    const char *instr, *cp, **parts;
    char *savestr, *deststr, *ncp;
    char curcolour[MAX_COLOUR_STRING];
    ...
    do {
        ...
        if (cp)
        {
            if (cp[1] == TC_SECOND_CHAR)
            {
                savestr = string_copy(instr, "f_terminal_color");
                cp = savestr + ( cp - instr );
                instr = savestr;
                break;
            }
    ...
    if (cp == NULL) {
        if (wrap) {
            num = 1;
            parts = (const char **)CALLOCATE(1, char *, TAG_TEMPORARY, "f_terminal_color: parts");
            parts[0] = instr;
            savestr = 0;
        } else {
            pop_stack(); /* no delimiter in string, so return the original */
        ...
        /* here we have something to parse */
        char *newstr = (char *) cp; //must be result of the string_copy above
        parts = (const char **) CALLOCATE(NSTRSEGS, char *, TAG_TEMPORARY, "f_terminal_color: parts");
        if (newstr - instr) {       /* starting seg, if not delimiter */
            num = 1;
            parts[0] = instr;
            *newstr = 0;
                ...
                *newstr = 0;
                ...
                if (newstr > instr) {
                    if (num && num % NSTRSEGS == 0) {
                        // Increase the size of the parts array.
                        parts = (const char **) RESIZE(parts, num + NSTRSEGS, char *,
                                       TAG_TEMPORARY, "f_terminal_color: parts realloc");
                    }
                    // Put it in at the current location in the parts array.
                    parts[num++] = instr;
                }
    ...
    /* Could keep track of the lens as we create parts, removing the need
       for a strlen() below */
    lens = CALLOCATE(num, int, TAG_TEMPORARY, "f_terminal_color: lens");
    mtab = sp->u.map->table;
    ...
    /* now we have the final string in parts and length in j.
       let's compose it, wrapping if necessary */
    ncp = deststr = new_string(j, "f_terminal_color: deststr");
    if (wrap) {
        char *tmp = new_string(max_buflen, "f_terminal_color: wrap");
        char *pt = tmp;
...
#ifndef DEBUG
    if (ncp - deststr != j) {
        fatal("Length miscalculated in terminal_color()\n    Expected: %i Was: %i\n    String: %s\n    Indent: %i Wrap: %i\n", j, ncp - deststr, sp->u.string, indent, wrap);
    }
#endif
    free_string_svalue(sp);
    sp->type = T_STRING;
