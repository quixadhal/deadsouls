...
{
/*
 * This code appears to be an attempt to catch invalid
 * object usage in the driver by setting the object pointer
 * to non-null values that will causes a segfault but be
 * identifyable.  This is only happening during extreme
 * stress testing which includes abrupt, non-clean disconnects.
 ...
 * I'm setting it to just clean up anyway since all of the code
 * that checks for dead objects compare the pointer to NULL.
 * 
 * Jun 11, 2008 - Kalinash
*/
